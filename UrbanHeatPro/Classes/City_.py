...
# --------------------------------------------------------------------------------
	def create_synthetic_city(self):
		...
		# initialize synthetic city dataframe
		syn_city_dict = {'bid' 					: np.zeros(len(self.buildings), dtype = int),
						 'footprint_area' 		: np.zeros(len(self.buildings)),
						 'use' 					: np.zeros(len(self.buildings), dtype = int),
						 'free_walls'			: np.zeros(len(self.buildings), dtype = int),
						 'lat' 					: np.zeros(len(self.buildings)),
						 'lon' 					: np.zeros(len(self.buildings)),
						 'dist_to_heat_source'  : np.zeros(len(self.buildings)),
						 'NUTS_0'				: np.array(['str' for _ in range(len(self.buildings))]), 
						 'NUTS_1' 				: np.array(['str' for _ in range(len(self.buildings))]), 
						 'NUTS_2' 			    : np.array(['str' for _ in range(len(self.buildings))]),
						 'NUTS_3' 			    : np.array(['str' for _ in range(len(self.buildings))]),
						 'year_class' 			: np.zeros(len(self.buildings), dtype = int),
						 'size_class'	   		: np.zeros(len(self.buildings), dtype = int),
						 'ref_level_roof'  		: np.zeros(len(self.buildings), dtype = int),
						 'ref_level_wall'  		: np.zeros(len(self.buildings), dtype = int),
						 'ref_level_floor' 		: np.zeros(len(self.buildings), dtype = int),
						 'ref_level_window'		: np.zeros(len(self.buildings), dtype = int),
						 'floors'          		: np.zeros(len(self.buildings)),
						 'area_corr_factor' 	: np.zeros(len(self.buildings)),
						 'storey_area'     		: np.zeros(len(self.buildings)),
						 'heated_area'     		: np.zeros(len(self.buildings)),
						 'dwellings'       		: np.zeros(len(self.buildings), dtype = int),
						 'dwelling_size'      	: np.zeros(len(self.buildings)),
						 'occupants'       		: np.zeros(len(self.buildings), dtype = int)}
		
		# convert to dataframe
		#self.syn_city = pd.DataFrame(syn_city_dict)	
		...
		self.syn_city = manager.dict(syn_city_dict)
				
		### set tasks for multiprocessing
		# Task: create synthetic building
		if (self.debug >= 1):
			print('   Setting tasks...')
		TASKS   = [(self.create_synthetic_building, 
					([self.buildings.loc[iii, :], iii])) for iii in range(len(self.buildings))]
		if (self.debug >= 1):
		...
		"""
		# Series simulation
		for iii, b in enumerate(self.buildings):
			print('Calculating bid: {}'.format(iii))
			jjj = self.create_synthetic_building(self.buildings.loc[iii, :], iii)
			input()
		"""
		
		# save results per city
		if (self.save >= 1):
			self.save_csv_syn_city()
#
	def create_synthetic_building(self, building, iii):
		"""
		...
		if iii == 0:
			...
			jjj = iii -1 
		#return iii, self.syn_city.loc[iii, 'bid'], self.syn_city.loc[jjj, 'bid']
		return iii, self.syn_city['bid'][iii], self.syn_city['bid'][jjj]
#										
	def append_building_to_synthetic_city(self, iii, building, dist_to_heat_source, year_class, size_class, 
											ref_level_roof, ref_level_wall, ref_level_floor, ref_level_window,
											floors, area_correction_factor, storey_area, heated_area,
											dwellings, dwelling_size, occupants):
		"""
		Adds building data to synthetic city matrix.
		Shared dictionary is copied every building.
		>>> Improve efficiency
		"""
		...
		# copy syn_city to temporal dict
		#for key in self.syn_city.keys():
		#	d[key] = self.syn_city[key][iii]
			
		d = self.syn_city
		
		# modify copy
		d['bid'][iii]			  = building.bid
		d['footprint_area'][iii]	  = building.footprint_area
		d['use'][iii]			  = building.use
		d['free_walls'][iii]	 	  = building.free_walls
		d['lat'][iii]			  = building.lat
		d['lon'][iii]			  = building.lon
		d['dist_to_heat_source'][iii] = dist_to_heat_source
		d['NUTS_0'][iii]			  = building.NUTS_0
		d['NUTS_1'][iii]			  = building.NUTS_1
		d['NUTS_2'][iii]			  = building.NUTS_2
		d['NUTS_3'][iii]		  = building.NUTS_3
		d['year_class'][iii]	 	  = year_class
		d['size_class'][iii]		  = size_class
		d['ref_level_roof'][iii]   = ref_level_roof
		d['ref_level_wall'][iii]   = ref_level_wall
		d['ref_level_floor'][iii]  = ref_level_floor
		d['ref_level_window'][iii] = ref_level_window
		d['floors'][iii]			  = floors
		d['area_corr_factor'][iii] = area_correction_factor
		d['storey_area'][iii]	  = storey_area
		d['heated_area'][iii]	  = heated_area
		d['dwellings'][iii]		  = dwellings
		d['dwelling_size'][iii]	  = dwelling_size
		d['occupants'][iii]		  = occupants
		
		# update original dictionary
		self.syn_city['bid'][iii]			  = d['bid'][iii]
		self.syn_city['footprint_area'][iii]	  = d['footprint_area'][iii]
		self.syn_city['use'][iii]			  = d['use'][iii]
		self.syn_city['free_walls'][iii]	 	  = d['free_walls'][iii]
		self.syn_city['lat'][iii]			  = d['lat'][iii]
		self.syn_city['lon'][iii]			  = d['lon'][iii]
		self.syn_city['dist_to_heat_source'][iii] = d['dist_to_heat_source'][iii]
		self.syn_city['NUTS_0'][iii]			  = d['NUTS_0'][iii]
		self.syn_city['NUTS_1'][iii]			  = d['NUTS_1'][iii]
		self.syn_city['NUTS_2'][iii]			  = d['NUTS_2'][iii]
		self.syn_city['NUTS_3'][iii]		  	  = d['NUTS_3'][iii]
		self.syn_city['year_class'][iii]	 	  = d['year_class'][iii]
		self.syn_city['size_class'][iii]		  = d['size_class'][iii]
		self.syn_city['ref_level_roof'][iii]   = d['ref_level_roof'][iii]
		self.syn_city['ref_level_wall'][iii]   = d['ref_level_wall'][iii] 
		self.syn_city['ref_level_floor'][iii]  = d['ref_level_floor'][iii]
		self.syn_city['ref_level_window'][iii] = d['ref_level_window'][iii]
		self.syn_city['floors'][iii]			  = d['floors'][iii]
		self.syn_city['area_corr_factor'][iii] = d['area_corr_factor'][iii]
		self.syn_city['storey_area'][iii]	  = d['storey_area'][iii]
		self.syn_city['heated_area'][iii]	  = d['heated_area'][iii]
		self.syn_city['dwellings'][iii]		  = d['dwellings'][iii]
		self.syn_city['dwelling_size'][iii]	  = d['dwelling_size'][iii]
		self.syn_city['occupants'][iii]		  = d['occupants'][iii]
#
	def append_building_to_synthetic_city_pd(self, iii, building, dist_to_heat_source, year_class, size_class, 
											ref_level_roof, ref_level_wall, ref_level_floor, ref_level_window,
											floors, area_correction_factor, storey_area, heated_area,
											dwellings, dwelling_size, occupants):
		"""
		...
		# copy dataframe
		c = self.syn_city.loc[iii, :].copy()
		
		# modify copy
		c['bid']			 	  = building.bid
		c['footprint_area'] 	  = building.footprint_area
		c['use'] 			 	  = building.use
		c['free_walls'] 	 	  = building.free_walls
		c['lat'] 			 	  = building.lat
		c['lon']			 	  = building.lon
		c['dist_to_heat_source']  = dist_to_heat_source
		c['NUTS_0'] 			  = building.NUTS_0
		c['NUTS_1'] 			  = building.NUTS_1
		c['NUTS_2'] 			  = building.NUTS_2
		c['NUTS_3'] 			  = building.NUTS_3
		c['year_class'] 	 	  = year_class
		c['size_class'] 		  = size_class
		c['ref_level_roof']  	  = ref_level_roof
		c['ref_level_wall']  	  = ref_level_wall
		c['ref_level_floor']  	  = ref_level_floor
		c['ref_level_window'] 	  = ref_level_window
		c['floors']			  	  = floors
		c['area_corr_factor']     = area_correction_factor
		c['storey_area'] 		  = storey_area
		c['heated_area']		  = heated_area
		c['dwellings'] 		  	  = dwellings
		c['dwelling_size'] 	      = dwelling_size
		c['occupants']		      = occupants
		
		# update original dataframe
		self.syn_city.loc[iii, :] = c
#	
...
#
	def save_csv_syn_city(self):
		...
		# Synthetic city
		#syn_city_df = pd.DataFrame(self.syn_city)
		
		# save
		#syn_city_df.to_csv(filename, sep = ";", float_format = '%.4f')
		self.syn_city.to_csv(filename, sep = ";", float_format = '%.4f')
		input()
#
