...
# Synthetic city
# --------------------------------------------------------------------------------
	def create_synthetic_city(self, chunk_size = 1000.):
		"""
		...
		# divide city into chunks
		no_chunks = len(self.buildings)/ chunk_size
		b 		  = np.arange(len(self.buildings))
		chunks 	  = np.array_split(b, no_chunks)
				
		# Create synthetic city for each chunk
		for iii, chunk in enumerate(chunks):
			
			if (self.debug >= 1):
				print('   Chunk {}/{}'.format(iii, len(chunks)))
		
			# initialize synthetic city dict
			syn_city_dict = {'bid' 			    : np.zeros(len(chunk), dtype = int),
						 'footprint_area' 		: np.zeros(len(chunk)),
						 'use' 					: np.zeros(len(chunk), dtype = int),
						 'free_walls'			: np.zeros(len(chunk), dtype = int),
						 'lat' 					: np.zeros(len(chunk)),
						 'lon' 					: np.zeros(len(chunk)),
						 'dist_to_heat_source'  : np.zeros(len(chunk)),
						 'NUTS_0'				: np.array(['str' for _ in range(len(chunk))]), 
						 'NUTS_1' 				: np.array(['str' for _ in range(len(chunk))]), 
						 'NUTS_2' 			    : np.array(['str' for _ in range(len(chunk))]),
						 'NUTS_3' 			    : np.array(['str' for _ in range(len(chunk))]),
						 'year_class' 			: np.zeros(len(chunk), dtype = int),
						 'size_class'	   		: np.zeros(len(chunk), dtype = int),
						 'ref_level_roof'  		: np.zeros(len(chunk), dtype = int),
						 'ref_level_wall'  		: np.zeros(len(chunk), dtype = int),
						 'ref_level_floor' 		: np.zeros(len(chunk), dtype = int),
						 'ref_level_window'		: np.zeros(len(chunk), dtype = int),
						 'floors'          		: np.zeros(len(chunk)),
						 'area_corr_factor' 	: np.zeros(len(chunk)),
						 'storey_area'     		: np.zeros(len(chunk)),
						 'heated_area'     		: np.zeros(len(chunk)),
						 'dwellings'       		: np.zeros(len(chunk), dtype = int),
						 'dwelling_size'      	: np.zeros(len(chunk)),
						 'occupants'       		: np.zeros(len(chunk), dtype = int)}
				
			# Multiprocessing
			### define multiprocessing pool
			if (self.debug >= 1):
				print('      ***')
				print('      Starting multiprocessing pool with {}/{} processors...'.format(self.processes, multiprocessing.cpu_count()))
			pool    = multiprocessing.Pool(self.processes)
			
			### Manager
			if (self.debug >= 1):
				print('      Starting shared dictionary...')
			...
			TASKS   = [(self.create_synthetic_building, 
						([self.buildings.loc[jjj, :], jjj])) for jjj in chunk]
			
			### assign tasks to processes
			if (self.debug >= 1):
				print('      Assigning tasks to processors...')
			...
			# save chunk results
			if (self.save >= 1):
				self.save_csv_syn_city(iii, self.syn_city.values())
#
...
#										
	def append_building_to_synthetic_city(self, iii, building, dist_to_heat_source, year_class, size_class, 
		...
		# modify copy
		d['bid']			  	 = building.bid
		d['footprint_area']  	 = building.footprint_area
		d['use']			  	 = building.use
		d['free_walls']	  		 = building.free_walls
		d['lat']			  	 = building.lat
		d['lon']			  	 = building.lon
		d['dist_to_heat_source'] = dist_to_heat_source
		d['NUTS_0']			  	 = building.NUTS_0
		d['NUTS_1']			  	 = building.NUTS_1
		d['NUTS_2']			  	 = building.NUTS_2
		d['NUTS_3']		 		 = building.NUTS_3
		d['year_class']	 	  	 = year_class
		d['size_class']		  	 = size_class
		d['ref_level_roof']  	 = ref_level_roof
		d['ref_level_wall']  	 = ref_level_wall
		d['ref_level_floor'] 	 = ref_level_floor
		d['ref_level_window'] 	 = ref_level_window
		d['floors']			  	 = floors
		d['area_corr_factor'] 	 = area_correction_factor
		d['storey_area']	  	 = storey_area
		d['heated_area']	  	 = heated_area
		d['dwellings']	  		 = dwellings
		d['dwelling_size']	  	 = dwelling_size
		...
		self.syn_city['bid'][iii]			  	  	= d['bid']
		self.syn_city['footprint_area'][iii]	  	= d['footprint_area']
		self.syn_city['use'][iii]			  		= d['use']
		self.syn_city['free_walls'][iii]	 	  	= d['free_walls']
		self.syn_city['lat'][iii]			  		= d['lat']
		self.syn_city['lon'][iii]			  		= d['lon']
		self.syn_city['dist_to_heat_source'][iii] 	= d['dist_to_heat_source']
		self.syn_city['NUTS_0'][iii]			  	= d['NUTS_0']
		self.syn_city['NUTS_1'][iii]			  	= d['NUTS_1']
		self.syn_city['NUTS_2'][iii]			  	= d['NUTS_2']
		self.syn_city['NUTS_3'][iii]		  	  	= d['NUTS_3']
		self.syn_city['year_class'][iii]	 	  	= d['year_class']
		self.syn_city['size_class'][iii]		  	= d['size_class']
		self.syn_city['ref_level_roof'][iii]   		= d['ref_level_roof']
		self.syn_city['ref_level_wall'][iii]   		= d['ref_level_wall']
		self.syn_city['ref_level_floor'][iii]  		= d['ref_level_floor']
		self.syn_city['ref_level_window'][iii] 		= d['ref_level_window']
		self.syn_city['floors'][iii]			  	= d['floors']
		self.syn_city['area_corr_factor'][iii] 		= d['area_corr_factor']
		self.syn_city['storey_area'][iii]	  		= d['storey_area']
		self.syn_city['heated_area'][iii]	  		= d['heated_area']
		self.syn_city['dwellings'][iii]		  		= d['dwellings']
		self.syn_city['dwelling_size'][iii]	  		= d['dwelling_size']
		self.syn_city['occupants'][iii]		  		= d['occupants']
#	
