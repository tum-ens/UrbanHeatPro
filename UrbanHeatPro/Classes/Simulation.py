...
# --------------------------------------------------------------------------------
	def read_input_data_csv(self):
		...
		V1_nres_csv  	= input_dir_ + 'AirFlowRate-1_NonResidential.csv'
		V2_nres_csv  	= input_dir_ + 'AirFlowRate-2_NonResidential.csv'
		V3_nres_csv  	= input_dir_ + 'AirFlowRate-3_NonResidential.csv'
		...
		ref_res_csv 	= input_dir_ + 'CurrentPercentageRefurbished_Residential_' + self.region + '.csv'
		max_ref_res_csv	= input_dir_ + 'MaxPercentageRefurbished_Residential_' + self.region + '.csv'

		## NON-RESIDENTIAL
		stock_nres_csv	= input_dir_ + 'BuildingStock_NonResidential_' + self.region + '.csv'
		ref_nres_csv 	= input_dir_ + 'CurrentPercentageRefurbished_NonResidential_' + self.region + '.csv'
		max_ref_nres_csv= input_dir_ + 'MaxPercentageRefurbished_Residential_' + self.region + '.csv'
		
		## BOTH
		Tset_csv  		= input_dir_ + 'Tset_' + self.region + '.csv'
		...
		V_USAGE_NR    = [self.read_data_from_csv(V1_nres_csv, usecols = 1), # L1
					 self.read_data_from_csv(V2_nres_csv, usecols = 1),   	# L2
					 self.read_data_from_csv(V3_nres_csv, usecols = 1)]   	# L3
		V_INF_NR      = [self.read_data_from_csv(V1_nres_csv, usecols = 2), # L1
					 self.read_data_from_csv(V2_nres_csv, usecols = 2),   	# L2
					 self.read_data_from_csv(V3_nres_csv, usecols = 2)]   	# L3
		...
		# >>> Read with geopandas
		CURRENT_REF_RES = self.read_data_from_csv(ref_res_csv, usecols = range(1, 5))
		...
		# >>> Read with geopandas
		MAX_REF_RES = self.read_data_from_csv(max_ref_res_csv, usecols = range(1, 5))
...
#
	def read_Tamb(self):
		...
		filename  = '{}/Regional Data/{}/Tamb_{}.csv'.format(self.my_dir, self.region, self.region) 
...
#
	def read_I(self):
		...
		filename  = '{}/Regional Data/{}/I_{}.csv'.format(self.my_dir, self.region, self.region) 
