	...
	def write_to_synthetic_city(self, writerQueue, filename):
		...
		while True:
			try:
				...
				np.savetxt(f, result, delimiter = ';', fmt = ['%d', '%.2f' ,'%d', '%d',
																'%.3f', '%.3f', '%.2f',
																'%d', '%d', 
																'%d', '%d', '%d', '%d', 
																'%d', '%.2f', '%.2f', '%.2f', 
...
#
	def create_synthetic_building(self, building):
		...
		result = [int(building.bid), building.footprint_area, building.use, building.free_walls,
				  building.lat, building.lon, dist_to_heat_source,
...
#	
	def calculate_number_of_floors_residential(self, year_class, size_class, area_correction_factor):
		...
		floors 	= int(np.ceil(self.FLOORS[year_class][size_class] / (1 + (1 - area_correction_factor))))
...
#
	def calculate_number_of_floors_non_residential(self, left = 1, mode = 2, right = 3):
		...
		floors = int(np.random.triangular(left, mode, right))
...
#
	def calculate_number_of_occupants_non_residential(self, use, heated_area):
		...
		# calculate number of occupants
		# buildings are assumed to be occupied at a capacity: 
		capacity = 0.1
		occupants_in_building = int(heated_area / area_per_person * capacity)
