...
# --------------------------------------------------------------------------------
	def __init__(self, dt_vector, resolution, heated_area, \
					...
					activity_vector, occupancy_vector, \
...
# --------------------------------------------------------------------------------		
	def calculate(self):
		...
		if self._solar_gains:
			...
			ORIENTATION = [-90 + deviation_from_south, 0 + deviation_from_south, 90 + deviation_from_south, -180 + deviation_from_south]
		...
		for iii in range(1, self.nts):
			...
			self.sh_powermax = (self.C / self.eta) * ((dT_in_resolution / dt_seconds) - \
							...
							(1. / self.C) * (self.solar_gains[iii - 1] + self.internal_gains[iii - 1]))
			...
			# limit input power
			if iii > 0:
				self.sh_power[iii] = (1. - self.thermal_intertia) * min(self.sh_power[iii], self.sh_powermax) + self.sh_power[iii - 1] * self.thermal_intertia
			else:
...
#
	def calculate_solar_gains(self, iii, RED_FACTORS, ORIENTATION):
		...
		for window in range(4): # windows are distributed in four orientations only
			...
			I_Gt   = self.calculate_incident_solar_irradiation(day_of_year, hour, I_Gh, I_Dh, I_ex, hs, lat, lon, slope, orientation)
			
			# calculate window area in specific orientation
			area   = self.window_areas[window]
					
			# calculate solar heat gain
			self.solar_gains[iii] += RED_FACTORS[0] * (1 - RED_FACTORS[1]) * RED_FACTORS[2] * RED_FACTORS[3] * area * I_Gt
#
	def calculate_incident_solar_irradiation(self, day_of_year, hour, I_Gh, I_Dh, I_ex, hs, lat, lon, slope, orientation):
		"""
		...
		if I_Gh > 0:
			...
			E = 3.82 * (0.000075 + 0.001868 * np.cos(B) - 0.032077 * np.sin(B) - 0.014615 * np.cos(2 * B) - 0.04089 * np.sin(2 * B))
			...
			incidence_angle = np.arccos(np.sin(declination_angle * np.pi / 180.) * np.sin(lat * np.pi / 180.) * np.cos(slope * np.pi / 180.) - \
				np.sin(declination_angle * np.pi / 180.) * np.cos(lat * np.pi / 180.) * np.sin(slope * np.pi / 180.) * np.cos(orientation * np.pi / 180.) + \
				np.cos(declination_angle * np.pi / 180.) * np.cos(lat * np.pi / 180.) * np.cos(slope * np.pi / 180.) * np.cos(hour_angle * np.pi / 180.) + \
				np.cos(declination_angle * np.pi / 180.) * np.sin(lat * np.pi / 180.) * np.sin(slope * np.pi / 180.) * np.cos(orientation * np.pi / 180.) * np.cos(hour_angle * np.pi / 180.) + \
				np.cos(declination_angle * np.pi / 180.) * np.sin(slope * np.pi / 180.) * np.sin(orientation * np.pi / 180.) * np.sin(hour_angle * np.pi / 180.)) * (180. / np.pi)
			...
			# Modulating function
			# Function to become an all sky-model (Klucher model) - Used to account for horizontal brightening and related to cloudiness
			F = np.sqrt(abs((I_Gh - I_Dh)) / I_Gh) 

			# Diffuse radiation (tilted), I_Dt [W/m2]    
			if (incidence_angle > 90):
				I_Dt = I_Dh * (1 - A_i) * (0.5 * (1 + np.cos(slope * np.pi / 180.))) * ( 1 + F * ((np.sin(slope / 2 * np.pi / 180.)) ** 3.))
			else:
				I_Dt = I_Dh * (1 - A_i) * (0.5 * (1 + np.cos(slope * np.pi / 180.))) * (1 + F * ((np.sin(slope / 2  * np.pi / 180.)) ** 3.)) + A_i * R_B 
